...
public class ArrayUtil {
    ...
    // 파라미터로 넘어온 int배열에서 같이 넘어온 숫자가
    // 존재하는지 존재하지 않는지
    // 존재하면 true 존재하지않으면 false를 리턴하는
    // contains() 메소드
    public static boolean contains(int[] intArray, int number) {

        // for문을 이용해서
        // 만약 intArray[i]의 number가 같으면
        // 그때만 return true 해준다.
        for (int i = 0; i < intArray.length; i++) {
    ...
    public static int[] removeByIndex(int[] array, int index) {
        ...
        // C. 임시 배열에서 i번째 인덱스를 i-1에 넣는 식으로
        // 복사해온다.
        // D. 줄어든 배열을 리턴한다.
