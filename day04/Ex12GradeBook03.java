...
// 또한 사용자가 어떤 기능을 실행할지 선택가지만 가능하다.
    ...
    static void showMenu(Scanner scanner) {
        ...
        int math = 0;
        while (true) {
            System.out.println("1. 성적입력 2.성적 출력 3. 종료");
    ...
    static void printGrade(int id, String name, int korean, int english, int math) {
        ...
        if (korean == -1) {
            System.out.printf("아직 입력된 정보가 없습니다");
        } else {
...
// call by value: 변수를 넘겨주더라도 값만 넘어가지 변수 그자체는 넘어가지않음
// call by reference: 주소값을 넘겨주기 때문에 변경을 가하면 변수에도 변경이 가해짐

// call by value의 경우, 해당 변수와 "값 복사본"이 넘어가고, 따라서
...
// call by value는 기본형 데이터타입 변수가 파라미터로 넘어갈 경우 발생하고
