...
/*
 *  메쏘드
 *  매쏘드란 자바에서 해당 클래스가 가지고 있는 기능을 뜻한다
 *  어떤 특성한 작업을 반복적으로 다른데서도 하게 되면
 *  그 코드를 처음부터 끝까지 다시 써주는 것은 비효율적이고 귀찮은 노동이 된다.
 *  
 *  그래서 그렇게 특정 기능들을 다시 사용해야한다면, 즉 그 코드들을 다시 사용해야한다면
 *  기능으로 분리해서 메소드를 만들면, 우리가 그 메소드를 호출만 해주면
 *  해당 코드들이 볼러와지기 때문에
 *  다시 안써줘도 된다!
 *  
 *  메쏘드는 선언과 구현으로 이루어져있다.
 *  선언 : 해당 메소드의 이름, 그리고 실행할때 필요로하는 외부의 값, 그리고 이 메소드가 종료되면서
 ...
 *  }
 *           
 *  리턴타입 : 해당 메소드가 종료되면서 이 메소드를 호출한 곳으로 보내줄 값의
 *           데이터타입을 말한다
 ...
 * 메소드 오버로딩이란, 이름은 같지만 파라미터가 달라져서 같은 목적을가진
 * 여러개의 메소드를 같은 이름으로 묶는 방법을 말한다.
 * 예를들어, System.out.println()은 총 9가지의 파라미터에 대해서
 * 오버로딩이 되어있다.
 * 
 ...
 *  default: 해당 메소드 혹은 필드는 같은 패키지의 다른 클래스들은 자유롭게 접근이 가능하지만
