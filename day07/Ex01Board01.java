...
//1.메뉴를 출력하는 showMenu()메소드
    private static void showMenu() {
        // 게시물을 관리할 board배열 boardArray
...
// addBoard()메소드
    private static Board[] add(Scanner scanner, Board[] boardArray) {
        ...
        // ArrayUtil에서 Board배열에 Board 변수를 추가해주는 메소드를 호출해서
        // 새로운 게시글을 읽어주자

        return ArrayUtil.add(boardArray, b);
        // = ~~해서
        // return boardArray;(같음)
    }

//파라미터로 넘어온 Board[]를 간단하게만 쭉 출력해주고
// 그 후에 사용자로부터 입력을 받아서
// 추가적인 작업을 결정하는
// selectAll()메소드
    private static Board[] selectAll(Scanner scanner, Board[] boardArray) {
        // 만약 boardArray안에 아무런 글이 없으면 경고 메시지만 출력하고
        // 끝낸다.
        ...
        // 간략하게 글번호, 제목정도만 출력을 쭉 해준다.
        for (int i = 0; i < boardArray.length; i++) {
            System.out.printf("%d.  %s\n", i + 1, boardArray[i].getTitle());
        }
        int userChoice = ScannerUtil.nextInt(scanner, "글 번호를 선택해주세요(0은 뒤로가기)", 0, boardArray.length) - 1;
    ...
    //파라미터로 넘어온 Board배열에서 해당 인덱스를 삭제하고 다시
    //호출된 곳으로 배열을 보내주는
    //deleteBoard()
