 ...
 * 1. 클래스
 *    클래스는 하나의 프로그램이다.
 *    좀 더 자세히 말하면 아나릐 프로그램 설계도이다.
 *    객체지향프로그래밍, 3세대 언어에서는
 *    하나의 커다란 프로그램이 작은 프로그램들을 활용해서
 *    '필요한 코드의 양을 줄이자'가 바로 3세대 언어, 객체지향언어가 목표하는바이다.
 *    
 * 2. 객체
 *    객체란 클래스 변수를 뜻한다.
 ...
 *    
 * 4. 필드
 *    필드란 해당 객체가 내부적으로 가지고 있는 데이터를 할당 할 공간을 뜻한다.
 *    변수처럼 
 *    데이터타입 필드이름
 *    으로 선언한다
 ...
 *    스텍 영역에 해당 변수 공간이 마련되고 주소값이 부여되었지만
 *    힙 영역에 해당 주소에는 아무것도 없는 상태를 널이라고 한다.
 ...
 *       그 외에 메소드나 공간은 확보가 되어있는 상태이다! 즉 nullPoinerException은
 *       발생하지 않는다.
 *    B. String string = null;
 *       이 코드에서는 String 클래스 객체 string이 선언 되었고 heap영역의 메모리주소값까지는
 ...
 *      String으로 뽑아내랴한다면 당연히 toString() 메소드를 오버라이드 해야한다!
 *    B. equals()
 ...
 *       
 *       public bloolean equals(Object o){
 *           return this == o;
 *        }
 * 10. 클래스의 캡슐화와 getter/setter
 *     캡슐화란, 클래스의 모든 필드 그리고 외부가 꼭 알아햐하는 몇몇 메소드를 제외하고서는
 ...
 *     setter 하는 메소드를 이용하여 값을 저장하고
 *     필드에 저장된 값을 호출할 때에는
 *     getter라는 메소드를 통하여 값을 호출한다.
 *     
 *     setter 메소드는 다음과 같은 양식을 가진다. 
 *     public void set필드의 이름(필드타입 필드이름){
 *     this.필드이름 = 필드이름;
 *     }
 *     
