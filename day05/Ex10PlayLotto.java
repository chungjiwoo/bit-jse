...
public class Ex10PlayLotto {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < SIZE; i++) {
            // 1. 1~45까지의 숫자
            int number = ScannerUtil.nextInt(scanner, "1~45까지의 숫자: ", 1, MAX);
            // 2. 해당 숫자가 배열에 존재하면 다시 입력하도록 한다.
            while (ArrayUtil.contains(userNumbers, number)) {
                System.out.println("중복된숫자입니다!");
                number = ScannerUtil.nextInt(scanner, "1~45까지의 숫자", 1, MAX);
            }
        ...
        // 그렇다면 우리가 어떻게 해야
        // 정확하게 점수를 계산할수 있을까?
        // ArrayUtil.conrtains()를 사용하여
        // contains(computerNumbers,userNumbers[i])가 true가 나올떄마다
        // 점수를 1점씩 올리면 된다
