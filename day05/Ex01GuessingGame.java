...
public class Ex01GuessingGame {
    ...
    private static void printScore(int score) {
        if (score == 0) {
            System.out.println("아직플레이한기록이없습니다");
        } else {
            System.out.printf("현재 최고기록: %02d 회\n", score);
        }
    }

    // 현재 점수와 최고점수를 비교해서 더 작은 숫자를
    // return 해주는 setBestScore 메소드

    private static int setBestScore(int score, int bestScore) {
        if (bestScore == 0) {
            System.out.println("맨 처음 플레이 하셔서 최고기록을 달성하셨습니다.");
            return score;
        }
        if (score < bestScore) {
            System.out.println("신기록달성");
            return score;
        } else {
            return bestScore;
        }
