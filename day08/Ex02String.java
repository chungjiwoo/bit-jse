...
public static void main(String[] args) {
    ...
    // 6. 만약 String이 공백으로만 이루어졌는지 확인할때에는
    //    isBlank()를 실행하면 된다
    str = "     ";
    System.out.println("str.inBlank():"+str.isBlank());
    str = "abc";
    System.out.println("str.inBlank():"+str.isBlank());
    
    
    // 7. 만약 String이 빈 문자열, 즉 아무런 문자도 없는지 확일할 때에는
    //    isEmpty()를 실행하면 된다.
    str = new String();
    ...
    //8. 만약 문자열에서 파라미터가 가장 마지막으로 나온 위치가 알고 싶을때에는
    //   lastIndexOf()를 실행하면 된다.
    //   만약 파라미터가 등장하지 않으면 -1이 리턴된다.
    ...
    pattern = "\\w*";   // \\w은 모든 문자가 알파벳과 숫자로 이루어져있는지를 확인함
    System.out.println("str.matches(pattern): "+str.matches(pattern));
    ...
    // 12. 문자열을 쪼갤떼에는 substring()을 사용한다.
    //     ****주의**** 쪼갠다고 해서 원본이 바뀌지는 않음
    
    //파라미터가 int 1개이면 해당 인덱스부터 문자열이 끝날때까지 분리함
    ...
    str ="       1        \n";
    System.out.println("trim() 전 srt: "+str);
    System.out.println("trim() 후 srt: "+str.trim());
    System.out.println("-----------------");
